...
@section('title', 'CLEARN │ Perbarui Kursus')
            ...
            <div class="card-bg rounded-3xl p-6 lg:p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    ...
                    <div>
                        ...
                        @php $isFree = old('course_price', $course->course_price) == 0; @endphp

                        <div class="flex gap-3">
                            <button type="button" id="btn_paid"
                                onclick="setCourseType('paid')"
                                class="flex-1 py-3 rounded-xl text-xs font-bold border-2 transition-all duration-200 {{ !$isFree ? 'bg-primary text-white border-primary shadow-md shadow-primary/20' : 'bg-white dark:bg-[#1A1625] text-slate-500 dark:text-slate-400 border-slate-200 dark:border-white/10 hover:border-primary/50' }}">
                        ...
                        <input type="hidden" id="course_type" name="course_type" value="{{ old('course_type', $isFree ? 'free' : 'paid') }}">
                        <input type="hidden" id="actual_price" name="course_price" value="{{ old('course_price', $course->course_price) }}">

                        {{-- Input Harga (muncul jika Berbayar) --}}
                        <div id="price_section" class="mt-4 {{ $isFree ? 'hidden' : '' }}">
                            <label class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 block">
                                Harga (RP)
                            </label>
                            ...
                            <div class="relative">
                                ...
                                <input type="text"
                                    id="display_price"
                                    value="{{ !$isFree ? rtrim(rtrim(number_format(old('course_price', $course->course_price), 2, '.', ''), '0'), '.') : '' }}"
                        ...
                        {{-- Pesan Gratis --}}
                        <div id="free_section" class="mt-4 {{ $isFree ? '' : 'hidden' }}">
