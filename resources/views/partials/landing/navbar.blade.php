<nav class="nav-landing bg-white dark:bg-[#1A1625] border-b border-gray-200 dark:border-[#2d2644] flex items-center justify-between px-6 lg:px-12 py-4">
    ...
    <div class="flex items-center gap-3">
        ...
        <button
            ...
            class="hidden md:inline-block bg-gradient-to-r from-[#A487F8] to-[#8B6FE8] text-white px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90">
            Daftar
        </button>
        @endauth
    </div>
...
<div id="mobile-nav-overlay" class="fixed inset-0 z-50 hidden md:hidden">
    ...
    <div id="mobile-nav-panel" class="absolute top-0 right-0 h-full w-72 max-w-[85vw] bg-white dark:bg-[#1A1625] border-l border-gray-200 dark:border-[#2d2644] shadow-2xl translate-x-full transition-transform duration-300">
        ...
        <div class="border-t border-gray-100 dark:border-[#2d2644] p-5 space-y-3">
            ...
            <button onclick="window.location='{{ route('choose_role') }}'" class="block w-full text-center px-4 py-3 rounded-xl text-sm font-semibold bg-gradient-to-r from-[#A487F8] to-[#8B6FE8] text-white hover:opacity-90 transition">
                Daftar
            </button>
            @endauth
        </div>
